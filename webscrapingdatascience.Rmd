...
```{r}
#Test with Elon Musk. 
elonmusk_df <- search_users("elon musk", n = 10, parse = TRUE, token = NULL, verbose = TRUE)
elonmusk_df
...
for (i in 1:length(data_df)) 
  ...
  tryCatch({data_twitter[i, c("user_id")] <- search_users(data_df[i, "name"], n=5, parse=TRUE, token=NULL, verbose=TRUE)
    }, error=function(e){cat("ERROR :", conditionMessage(e), "\n")})
