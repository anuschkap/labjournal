...
#Now I want to build a loop to find the user names based on the names. I want to save screen_name & followers_count. Im not sure how to save those outcomes. 
data_twitter$user_id <- ""
for (i in 1:length(data_df)) {
  ...
  if (data_df$name [i]) {
    data_twitter <- search_users("i", n=5, parse=TRUE, token=NULL, verbose=TRUE)
  }
}
...
for (i in 1:length(data_df)) 
  ...
  tryCatch({data_twitter[i, c("user_id")] <- search_users(data_df[i, "name"], n=1, parse=TRUE, token=NULL, verbose=TRUE)
    }, error=function(e){cat("ERROR :", conditionMessage(e), "\n")})
...
for (i in 1:length(data_df)) 
  ...
  tryCatch({data_twitter[i, c("id_str"|"screen_name"| "followerscount")] <- search_users(data_df[i, "name"], n=1, parse=FALSE, token=NULL, verbose=TRUE)
    }, error=function(e){cat("ERROR :", conditionMessage(e), "\n")})   
