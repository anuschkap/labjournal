...
```
Here we can see that the distribution is still skewed, but that there are now a lower number of staff members with a low KI, given that they have Twitter. Still, most of the staff members have a low KI. 
...
```{r}
#I don't have pubyears here yet, I have to make that later and include it here. 
modelki2 <- lm(ki ~ gender + dutch, data=datadef_df)
summary(modelki2)
```
At the data science department, we see that the KI is predicted way differently than at the sociology department. The effect of gender is negative, meaning that men have a lower KI than women, but this effect is not significant. The effect of ethnicity is also not significant. 
